...
class MultiheadAttention_CustomAttn(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., custom_attn="cosine_similarity", 
        ...
        if incepattn_bot:
            self.inception=True
            self.q_conv = Inception1d(input_channels=embed_dim,use_residual=True, kernel_size=kernel_size, 
            ...
            self.k_conv = Inception1d(input_channels=embed_dim,use_residual=True, kernel_size=kernel_size, 
                                      kernel_sizes=kernel_sizes, dilation_sizes=dilation_sizes, depth=incepattn_dep, 
                                      bottleneck_size=incepattn_bot, nb_filters=incepattn_nb,bn_running=False, maxpool=incepattn_mp) 
        else:
            ...
            if q_k_func:
                ...
                if output_dim:
                    self.q_k_conv = nn.Conv1d(embed_dim, output_dim*2, kernel_size=kernel_size, padding=(kernel_size-1)//2)
                else:
                    self.q_k_conv = nn.Conv1d(embed_dim, embed_dim*2, kernel_size=kernel_size, padding=(kernel_size-1)//2)
        
        if output_dim:
            self.v_linear = nn.Linear(embed_dim, output_dim) #conv w/ ks=1
        else:
            self.v_linear = nn.Linear(embed_dim, embed_dim) #conv w/ ks=1
...
class InceptionBlock1d(nn.Module):
    def __init__(self, in_channels, nb_filters, kss, stride=1, act='linear', bottleneck_size=32, bn_running=True, maxpool=True, dilation_sizes=None):
        super().__init__()
        if bottleneck_size > 0:
            ...
            if dilation_sizes:
                self.convs = nn.ModuleList([nn.Conv1d(bottleneck_size, nb_filters, kernel_size=ks, dilation=ds, stride=stride, padding=(ks-1)//2*ds, bias=False)
            ...
            if dilation_sizes:
                self.convs = nn.ModuleList([nn.Conv1d(in_channels, nb_filters, kernel_size=ks, dilation=ds, stride=stride, padding=(ks-1)//2*ds, bias=False)
                    for ks, ds in zip(kss,dilation_sizes)])
            else:
...
class InceptionBackbone(nn.Module):
    def __init__(self, input_channels, kss, depth, bottleneck_size, nb_filters, use_residual, bn_running=True, maxpool=True, dilation_sizes=None):
        super().__init__()
        self.depth = depth
...
class Inception1d(nn.Module):
    ...
    def __init__(self, input_channels=8, kernel_size=40, kernel_sizes=None, dilation_sizes=None, 
        ...
        self.inception = InceptionBackbone(input_channels=input_channels, kss=kernel_size, depth=depth, dilation_sizes=dilation_sizes,
